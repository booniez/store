...
public class TenantBroker {
	 ...
	 * @param runAs    业务逻辑
	 */
	public void runAs(Long tenantId, RunAs<Long> runAs) {
	 ...
	 * @return 业务返回值
	 */
	public <T> T applyAs(Long tenantId, ApplyAs<Long, T> applyAs) {
		final Long pre = TenantContextHolder.getTenantId();
		try {
			log.trace("TenantBroker 切换租户 {} -> {}", pre, tenantId);
			TenantContextHolder.setTenantId(tenantId);
	 ...
	 * @param runAs          业务逻辑
	 */
	public void runAs(Supplier<Long> tenantSupplier, RunAs<Long> runAs) {
	 ...
	 * @return 业务返回值
	 */
	public <T> T applyAs(Supplier<Long> tenantSupplier, ApplyAs<Long, T> applyAs) {
