...
package com.storehub.store.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import com.storehub.store.common.xss.config.StoreXssProperties;
import com.storehub.store.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final StoreXssProperties properties;

	public DefaultXssCleaner(StoreXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(StoreXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		StoreXssProperties.Mode mode = properties.getMode();
		if (StoreXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (StoreXssProperties.Mode.validate == mode) {
			// 校验
